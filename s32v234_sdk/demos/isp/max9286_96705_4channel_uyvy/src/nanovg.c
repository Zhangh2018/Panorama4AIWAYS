...
void nvgFill(NVGcontext* ctx)
{
	...
	ctx->params.renderFill(ctx->params.userPtr, &fillPaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
						   ctx->cache->bounds, ctx->cache->paths, ctx->cache->npaths, state->transform, state->transformCallback, state->transformCallbackCtx);
...
void nvgStroke(NVGcontext* ctx)
{
	...
	ctx->params.renderStroke(ctx->params.userPtr, &strokePaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
							 strokeWidth, ctx->cache->paths, ctx->cache->npaths, state->transform, state->transformCallback, state->transformCallbackCtx);
...
static void nvg__renderText(NVGcontext* ctx, NVGvertex* verts, int nverts)
{
	...
	ctx->params.renderTriangles(ctx->params.userPtr, &paint, state->compositeOperation, &state->scissor, verts, nverts,
		state->transform, state->transformCallback, state->transformCallbackCtx);
